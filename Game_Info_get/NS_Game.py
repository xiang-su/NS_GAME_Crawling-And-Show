...
def Get_Rate():
    ...
    html = requests.get(url).content.decode(
        'utf8')  # 获取网页源码（中间涉及到编码问题,这是个大坑，你得自己摸索）
...
# 解析主源码导入
fl = open('E:/Github/NS_GAME_Crawling-And-Show/Game_Info_get/ceshi.txt', "r", encoding='gbk')
HtmlSource = fl.read()
HtmlSource.replace(' ', '')
...
def getGame(Text):
    ...
    for index in range(len(infolist)):
        ...
        Price_Best_infolist_1 = re.findall(
            Price_Best_info_1, str(Price_all_infolist))
        # print(Price_Best_infolist_1)
        # 获取最优价格,第二次筛选
        reg_Price_Best_2 = '"regular":.*?},'
        Price_Best_info_2 = re.compile(reg_Price_Best_2)
        Price_Best_infolist_2 = re.findall(
            Price_Best_info_2, str(Price_Best_infolist_1))
        Price_Best_infolist_2 = str(
            Price_Best_infolist_2).replace('\'"regular":', '')
        ...
        # 汇率转换，输出最优价格
        Price_Best_infolist_3 = '%.1f' % float(
            EX_Rate * float(Price_Best_infolist_2))
        ...
        Price_Best_country_infolist = re.findall(
            Price_Best_country_info, str(Price_all_infolist))
        Price_Best_country_infolist = str(
            Price_Best_country_infolist).replace('"country_name":"', '')
        Price_Best_country_infolist = str(
            Price_Best_country_infolist).replace('",', '')
        Price_Best_country_infolist = str(
            Price_Best_country_infolist).replace('[]', '0')
        Price_Best_country_infolist = str(
            Price_Best_country_infolist).replace('[', '')
        Price_Best_country_infolist = str(
            Price_Best_country_infolist).replace(']', '')
    ...
    # 解析结果显示
    print('  ' + myAlign('序号', 5) + myAlign('游戏名称', 55) + myAlign('发售日期', 15) + myAlign('详情地址', 36) +
          myAlign('公布状况', 10) + myAlign('具有试玩', 10) + myAlign('即将上市', 10) + myAlign('最优价格', 20) + myAlign('最优价格区服', 20))
    for index in range(len(datelist)):
        print('  ' + myAlign(str(index + 1), 5) + myAlign(namelist[index], 60) + myAlign(datelist[index], 20) + myAlign(infolist[index], 40) + myAlign(re_infolist[index], 10) + myAlign(
            has_demo_infolist[index], 10) + myAlign(coming_soon_infolist[index], 10) + myAlign(Game_Best_Price[index], 20), myAlign(Game_Best_Price_Country[index], 20))
    ...
    for index in range(210, len(imglist)):
        ...
        if len(jN) > 0:
            urllib.request.urlretrieve(
                imglist[index], 'c:\\NS_Game\\' + infolist[index] + '.jpg')
        else:
            if len(pN) > 0:
                urllib.request.urlretrieve(
                    imglist[index], 'c:\\NS_Game\\' + infolist[index] + '.jpg')
